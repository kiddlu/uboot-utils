 ...
 *
 * This code is derived from the uboot-envtools version 20081215:
...
#endif
		if (write(fd, data + processed, erasesize) != (int)erasesize) {
...
static int flash_read(int fd)
{
	...
	return (rc != (int)CONFIG_ENV_SIZE) ? -1 : 0;
}
...
static int env_init(void)
{
	...
	crc0_ok = (crc0 == (int)*environment.crc);
	fprintf(stderr, "[crc0 data:%8x flash:%8x!]\n", (int)*environment.crc, crc0);
	if (!HaveRedundEnv) {
		if (!crc0_ok) {
			fprintf(stderr,
				"Warning: Bad CRC detected, ignoring!\n");
		}
	} else {
		...
		crc1_ok = (crc1 == (int)redundant->crc);
		fprintf(stderr, "[crc1 data:%8x flash:%8x!]\n", (int)redundant->crc, crc1);
...
static int parse_config()
{
	...
	if (get_config(CONFIG_FILE)) {
		fprintf(stderr,
			"Cannot parse config file (" CONFIG_FILE "): %s\n", strerror(errno));
		return -1;
	}
#endif
